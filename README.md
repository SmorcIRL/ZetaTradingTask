...
## Notes
1) I've used SQL Server. The database name is `ZetaTradingTask_TM` and it is created automatically on startup.
2) The application URLs for the local environment are `https://localhost:7081` `http://localhost:5174`.
3) I decided not to add request validation, just to be able to test the required functionality with exception logging. In a real app I'd use FluentValidation or a similar library.
